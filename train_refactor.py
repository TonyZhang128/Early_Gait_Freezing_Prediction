...
def parse_args():
    ...
    # 数据参数
    parser.add_argument('--data_path', type=str, default='data2/all_data.mat',
                        help='训练数据路径')
    ...
    def get_transforms(cls, freq_keep_ratio=0.6):
        ...
        return transforms.Compose([
            transforms.RandomApply([
                ...
                transforms.RandomErasing(p=1.0, scale=(0.2, 0.4), ratio=(0.3, 3.3), value=0),
...
class SimCLRModel(nn.Module):
    ...
    def __init__(self, base_model, out_dim=32, proj_out_dim=128, dropout=0.5):
        """
        Args:
...
def create_model(device, out_dim=32, proj_out_dim=128, dropout=0.5):
    """创建模型实例"""
    model = SimCLRModel(ResNet101(), out_dim=out_dim, proj_out_dim=proj_out_dim, dropout=dropout).to(device)
    return model
...
def train(args):
    ...
    model = create_model(device, args.out_dim, args.proj_out_dim, args.dropout)
