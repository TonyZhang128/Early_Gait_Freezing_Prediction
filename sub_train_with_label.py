...
data_transforms = transforms.Compose([
    ...
    ], p=0.5)  # p=1.0 表示总是随机应用其中一种方法
])

random_index = np.array(range(0, len(scipy.io.loadmat('data3/sub_test_data.mat')['sub_data'])))  # 将数组打乱)
np.random.shuffle(random_index)
gait = sio.loadmat('data3/sub_test_data.mat')['sub_data'][random_index]
gaitlabel = sio.loadmat('data3/sub_test_label.mat')['sub_label'][0][random_index]
...
last_10_aucs = np.mean(test_aucs[-10:])

print(f'Average of last 10 epochs - Accuracy: {last_10_accs}, Precision: {last_10_precisions}, Recall: {last_10_recalls}, F1: {last_10_f1s}, auc: {last_10_aucs}')

# 保存模型
#if epoch + 1 == 200:  # 检查是否完成了200个epoch
#    model_path = "./save_model/final_model_after_teacher.pth"  # 定义模型保存路径
