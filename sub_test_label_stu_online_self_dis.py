...
for p in encoder_t.parameters():                    # 梯度冻结
...
opt = torch.optim.Adam(list(student.parameters()) , lr=3e-4)  # ② 一起优化
...
alpha   = 0.7          # 交叉熵权重
epochs  = 200
ema_decay = 0.99          # ←←← 新增：EMA 动量

#现在是自蒸馏在线

def online_distill_loss(logits_s, logits_t, y_true, alpha, T):
    L_ce = F.cross_entropy(logits_s, y_true)                                # 学生学真标签
    L_kd = F.kl_div(F.log_softmax(logits_s/T, dim=1),
...
for epoch in range(epochs):
    ...
    for x, y in train_ld:    # 不再需要 soft_idx
        x, y = x.to(device, dtype=torch.float32), y.to(device)
        opt.zero_grad()
        x = torch.squeeze(x, dim=1)
        logits_s = student(x)
        with torch.no_grad():                # 教师只推理
            logits_t = teacher(x)

        loss = online_distill_loss(logits_s, logits_t, y, alpha, T)
