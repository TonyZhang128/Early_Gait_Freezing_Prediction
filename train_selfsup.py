...
from utils.analysis import save_checkpoint, set_seed
...
def build_model(model_type: str, args, device: str):
    ...
    model = SimCLRModel(
        encoder, args.out_dim, args.proj_out_dim, args.contrastive_dim, args.dropout
    )
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Gait self-supervised pre-training with SimCLR"
    )
    ...
    parser.add_argument(
        ...
        help="debug: run 1 batch per epoch",
    )

    # Data
    parser.add_argument(
        "--data_path", type=str, default="datasets/data_10000/all_data.mat"
    )
    parser.add_argument("--batch_size", type=int, default=256)
    parser.add_argument(
        "--num_workers", type=int, default=0, help="0 in win, 4 in Linux"
    )
    parser.add_argument(
        "--views", type=int, default=2, help="Number of views for contrastive learning"
    )
    ...
    parser.add_argument(
        "--base_lr", type=float, default=3e-4, help="Base LR for batch size 256 (AdamW)"
    )
    ...
    parser.add_argument(
        "--weight_decay", type=float, default=0.01, help="AdamW decoupled weight decay"
    )
