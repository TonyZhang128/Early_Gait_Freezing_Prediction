...
opt = torch.optim.Adam(list(student.parameters()) +
                       list(teacher.parameters()), lr=3e-4)  # ② 一起优化
...
# 在线互相蒸馏（一步实现）
def mutual_loss(logits_s, logits_t, y_true, alpha, T):
    L_ce_s  = F.cross_entropy(logits_s, y_true)
    L_ce_t  = F.cross_entropy(logits_t, y_true)
    L_kl_st = F.kl_div(F.log_softmax(logits_s/T, dim=1),
                       F.softmax(logits_t/T, dim=1), reduction='batchmean')*(T*T)
    L_kl_ts = F.kl_div(F.log_softmax(logits_t/T, dim=1),
                       F.softmax(logits_s/T, dim=1), reduction='batchmean')*(T*T)
    return (alpha*(L_ce_s+L_ce_t) +
            (1-alpha)*(L_kl_st+L_kl_ts))
...
for epoch in range(epochs):
    ...
    for (x, y), idx_b in zip(train_ld, soft_idx):
        ...
        loss = mutual_loss(logits_s, logits_t, y, alpha, T)
