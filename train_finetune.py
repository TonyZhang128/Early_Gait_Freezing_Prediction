...
def train(args):
    ...
    print(
        f"Train: {len(train_loader.dataset)} samples, Test: {len(test_loader.dataset)} samples"
    )
    ...
    if (
        not args.supervised
        and args.pretrained_model
        and os.path.exists(args.pretrained_model)
    ):
    ...
    for epoch in range(args.num_epochs):
        ...
        test_metrics = (
            evaluate(model, test_loader, device, args.model_type)
            if test_loader
            else None
        )
        ...
        current_acc = (
            test_metrics["accuracy"] if test_metrics else train_metrics["accuracy"]
        )
