...
data_transforms = transforms.Compose([
    transforms.RandomApply([
        ...
        gait_reverse_transform      #aug3将每个信号沿x轴翻转（加负号）
    ], p=1.0)  # p=1.0 表示总是随机应用其中一种方法
])



gait = sio.loadmat('data2/all_data.mat')['all_data']
...
class SimCLRModel(nn.Module):
    def __init__(self, base_model, out_dim=132, proj_out_dim=128):
        ...
        self.projector = nn.Sequential(
            ...
            nn.Linear(proj_out_dim, 256)  # 投影回原始特征维度
        )
...
# 初始化模型和损失函数
model = SimCLRModel(ResNet101(), out_dim=32).to(device)
